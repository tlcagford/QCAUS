...
def pdp_entanglement_overlay(image: np.ndarray, interference: np.ndarray,
    ...
    if image.shape != interference.shape or image.shape != soliton.shape:
        st.error(f"Shape mismatch prevented! image={image.shape}, interf={interference.shape}, soliton={soliton.shape}")
        return np.zeros_like(image)
    m = omega * 0.6
    return np.clip(image * (1 - m * 0.4) + interference * m * 0.5 + soliton * m * 0.4, 0, 1)

def generate_interference_pattern(size: int, fringe: float, omega: float) -> np.ndarray:
    y, x = np.ogrid[:size, :size]
    cx, cy = size // 2, size // 2
...
def blue_halo_fusion(image: np.ndarray, dark_mode: np.ndarray, residuals: np.ndarray) -> np.ndarray:
    def pnorm(a):
        mn, mx = a.min(), a.max()
...
def magnetar_physics(size: int = 300, B0: float = 1e15, mixing_angle: float = 0.1) -> tuple:
    B_CRIT = 4.414e13
    y, x   = np.ogrid[:size, :size]
    cx, cy = size // 2, size // 2
    dx = (x - cx) / (size / 4)
...
def em_spectrum_composite(img_gray: np.ndarray, f_nl: float, n_q: float) -> np.ndarray:
...
def get_download_link(arr: np.ndarray, filename: str, label: str = "📥 Download", cmap: str = None) -> str:
...
st.markdown('<p style="text-align:center">FDM · PDP · Magnetar · QCIS · EM Spectrum</p>', unsafe_allow_html=True)
...
with col2:
    st.markdown("### — OR —")
    uploaded_file = st.file_uploader("Drag & drop file here", type=["jpg", "jpeg", "png", "fits"],
                                     help="Limit 200 MB per file", label_visibility="collapsed")
