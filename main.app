...
def plot_p3():
    ...
    S = -rho * np.log(rho) + 0.1 * np.sin(2*np.pi*rho)
    fig, ax = plt.subplots(figsize=(8, 4.5))
...
p_dark_bayes = min(1.0, (0.01 * p_dark_bayes) / (0.01 * p_dark_bayes + 0.99))

with col_met1:
    st.metric("P_dark Peak", f"{p_dark_bayes*100:.1f}%")
...
def log_prior(theta, ranges):
    for val, (low, high) in zip(theta, ranges):
        if not (low <= val <= high): return -np.inf
    return 0.0
...
if st.button("🚀 Compute Full Real-Physics Posterior (32 walkers × 5000 steps)", type="primary"):
    param_ranges = [(0.05, 0.50), (-12.0, -8.0), (0.10, 10.00), (10, 80), (13.00, 16.00)]
    ...
    for i, label in enumerate(labels):
        ...
        summary_data.append({"Parameter": label, "Median": f"{q50:.4f}", "+1σ": f"{q84-q50:.4f}", "-1σ": f"{q50-q16:.4f}"})
    ...
    with open("QCAUS_SwiftJ1818_Posterior.npy", "rb") as f:
        st.download_button("📥 Download full posterior samples", f, "QCAUS_Run047_posterior.npy")
...
formulas = {
    ...
    "Formula": [r"ρ(r)=ρ₀[sin(kr)/(kr)]²", r"dark_mask=ε·e^{-ΩR²}·sin(2πRL/f)", r"S=-ρ·log(ρ)+ψ_ord+ψ_dark",
                r"P=prior·L/(prior·L+(1-prior))", r"R=orig G=residuals B=dark γ=0.45", r"R=orig+P_dark G=FDM B=PDP",
                r"B=B₀(R/r)³√(3cos²θ+1) P=ε²(1-e^{-B²/m²})", r"P(k)=P_ΛCDM(k)×(1+f_NL·(k/k₀)^n_q)",
